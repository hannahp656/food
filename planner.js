...
// recompute shopping list whenever plan changes
function updateShoppingList() {
  ...
  Object.values(plan).forEach(meals =>
    Object.values(meals).forEach(items =>
      items.forEach(item => {
        if (item.link) {
          recipePromises.push(fetch(item.link)
            .then(res => res.text())
            .then(html => {
...
function addShoppingListHandlers() {
  shoppingListEl.querySelectorAll("li").forEach(li => {
    ...
    checkbox.addEventListener("change", () => {
      ...
      if (checkbox.checked) {
        shoppingListEl.appendChild(li); // move to bottom
      } else {
...
// load manual state on page load
